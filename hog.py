...
def free_bacon(score):
    ...
    if x < 10:
        ...
        return 1 + abs((x//100)-((x//10)%10)+(x%10))
    elif x >= 1000 and x < 10000:
        return 1 + abs((x//1000)-(((x//10)//10)%10)+((x//10)%10)-(x%10))
    elif x >= 10000 and x < 100000:
        return 1 + abs((x//10000)-((((x//10)//10)//10)%10)+(((x//10)//10)%10)-((x//10)%10)+(x%10))
    elif x >= 100000 and x < 1000000:
        return 1 + abs((x//100000)-(((((x//10)//10)//10)//10)%10)+((((x//10)//10)//10)%10)-(((x//10)//10)%10)+((x//10)%10)-(x%10))
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    y = pow(3,(player_score + opponent_score)) % 10
    if x >= 10:
        while x >= 10:
            x = x // 10
        if x == y:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            raw_score0 = take_turn(num_rolls0,score1,dice)
            score0 += raw_score0
            if feral_hogs == True:
                if abs(num_rolls0 - previous_score0)== 2:
                    score0 += 3
                previous_score0 = raw_score0
        elif who == 1:
            num_rolls1 = strategy1(score1,score0)
            raw_score1 = take_turn(num_rolls1,score0,dice)
            score1 += raw_score1
            if feral_hogs == True:
                if abs(num_rolls1 - previous_score1) == 2:
                    score1 += 3
                previous_score1 = raw_score1
...
def announce_highest(who, prev_high=0, prev_score=0):
    ...
    def say(score0,score1):
        ...
        if score_diff > prev_high:
            ...
            return announce_highest(who, prev_high, current_score)
    return say
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    while n <= 10:
        ...
        max_score = average_value(n, dice)
        if max_score > init_max:
            init_max = max_score
            max_roll = n
        n += 1
    return max_roll
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    while score >= 0 and opponent_score >=0:
        if free_bacon(opponent_score) >= margin:
            ...
            if is_swap(score, opponent_score) == True:
                if score < opponent_score:
                    ...
                    return num_rolls
            elif is_swap(score, opponent_score) == False:
                return 0
        else:
            score = free_bacon(opponent_score) + score
            if is_swap(score, opponent_score) == True:
